...
const WorkItem = ({ workItem = {}, workItems = [] }) => {
	...
	useInterval(
		() => {
			setScrubberPosition(
				(player.current.getCurrentTime() / totalPlaySeconds) *
					scrubberWidth
			)
		},
		isPlaying ? 75 : null
							...
							{!isIpad && (
								<div
									className={`${
										videoPlaying
											? "opacity-0"
											: "bg-trans-black opacity-100"
									} duration-500 absolute inset-0 flex flex-col items-center justify-center`}
								>
									<h2 className="text-2xl md:text-2xl lg:text-6xl uppercase space-x-4 md:space-x-8 text-center py-4">
									...
									>
										<BiPlayCircle
											className="bpd-play-icon lg:text-5xl my-6"
											color="white"
											size={90}
										/>
									</button>
									<h1 className="text-2xl md:text-2xl lg:text-4xl uppercase space-x-4 md:space-x-8 text-center py-4">
								...
								<button
									...
									onClick={(e) => {
										...
										const zeroBasedClickPosition =
											e.clientX -
											scrubberBoundingClientRect.left

										const xPercentageClicked =
											zeroBasedClickPosition /
										...
										setScrubberPosition(
											xPercentageClicked * scrubberWidth
										)
									}}
									ref={scrubber}
								>
